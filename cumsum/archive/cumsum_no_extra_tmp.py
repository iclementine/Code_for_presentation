...
def cumsum_loop_kernel(in_ptr, out_ptr, M, N, TILE_N: tl.constexpr):
    ...
    for start_n in range(0, N, TILE_N):
        ...
        offsets = pid * N + n_offsets
        x = tl.load(in_ptr + offsets, mask=mask, other=0)
        # load previous sum as a scalar
        previous_sum = tl.load(out_ptr + pid * N + start_n - 1, mask= start_n> 0, other=0)
...
def test_cumsum_loop():
    x = torch.randint(0, 2, (10, 256 * 1024), dtype=torch.int64, device="cuda")
    ref = torch.cumsum(x, 1)
    hyp = cumsum(x)
    torch.testing.assert_close(hyp, ref)
...
def test_cumsum_split():
    x = torch.randint(0, 2, (10, 1024 * 1024), dtype=torch.int64, device="cuda")
    ref = torch.cumsum(x, 1)
    hyp = cumsum(x)
    torch.testing.assert_close(hyp, ref)
