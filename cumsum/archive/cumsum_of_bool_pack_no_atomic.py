...
def chained_scan_decoupled_lookback_kernel(
    ...
    # NOTE: .cg cache modifier is not necessary
    tl.store(flag_ptr + pid_m * OUT_N + pid_n, pack)
    ...
    i = pid_n - 1
    while i >= 0:
        # NOTE: load with volatile introduces a threadd fence, it is enough
        pack_ = tl.load(flag_ptr + pid_m * OUT_N + i, volatile=True) # uint32
        while pack_ == 0:
            pack_ = tl.load(flag_ptr + pid_m * OUT_N + i, volatile=True) # uint32
        v = pack_ & v_mask
        exclusive_prefix += v
    ...
    tl.store(flag_ptr + pid_m * OUT_N + pid_n, pack2)
...
if __name__ == "__main__":
    x = torch.randint(0, 2, (4, 4096 * 1024), dtype=torch.bool, device="cuda:0")
    y = torch.cumsum(x, 1)
    #print(f"ref: {y}")
