"""
Triton < 3.3 has a bug and cannot produce correct result
"""
...
from triton_k_bit_global_hist import compute_global_hist_kernel, convert_to_uint_preverse_order
from utils import generate_matrix

@triton.jit
def sweep(arr_ptr, associate_arr_ptr, 
          out_ptr, associate_out_ptr, # outputs
          excumsum_bins_ptr, flag_ptr, aggregate_ptr, inclusive_prefix_ptr, # status
          n_passes, pass_id, bit_offset, N, OUT_N, 
          TILE_N: tl.constexpr, TILE_R: tl.constexpr, k_bits: tl.constexpr,
    ...
    for bin_index in tl.range(cta_r_start, cta_r_end, num_stages=2):
        ...
        local_ex_cumsum = tl.cumsum(matches.to(tl.int64), axis=0) - matches # (TILE_N, )
...
def radix_sort(arr, k_bits=8, descending=False):
    ...
    # test
    # global_hist_ref = torch.zeros((m, n_passes, num_bins), device=arr.device, dtype=torch.int64)
    # bfe_mask = (1 << k_bits) - 1
    # for i in range(n_passes):
    #     bit_offset = i * k_bits
    #     key = (arr >> bit_offset) & bfe_mask
    #     bins = torch.arange(0, 2 ** k_bits, dtype=torch.int32, device=arr.device) #(r, )
    #     matches = bins[:, None] == key.unsqueeze(1)  #(m, r, N)
    #     hist = torch.sum(matches, axis=-1) # m, r
    #     global_hist_ref[:, i, :] = hist
    # torch.testing.assert_close(global_hist, global_hist_ref)
    ...
    #         num_bins = 2 ** k_bits
    #         key = (arr >> bit_offset) & bfe_mask
    #         bins = torch.arange(num_bins, device=arr.device)
    #         matches = (bins[:, None] == torch.unsqueeze(key, 1))
    #         h = torch.sum(matches, -1)
    #         global_hist_ref[:, p, :] = h
    #     ex_cumsum_bins_ref = torch.cumsum(global_hist_ref, -1) - global_hist_ref
    #     torch.testing.assert_close(global_hist, global_hist_ref)
    #     torch.testing.assert_close(ex_cumsum_bins, ex_cumsum_bins_ref)
    # end{validation}
    ...
    aggregate = torch.empty((m, num_bins, grid_n), device=arr.device, dtype=torch.int64)
    inclusive_prefix = torch.empty((m, num_bins, grid_n), device=arr.device, dtype=torch.int64)

    for i in range(0, n_passes):
        bit_offset = i * k_bits
        flag.zero_() # must zero init it.
        sweep[grid_for_sweep](arr_in, indices_in, arr_out, indices_out, ex_cumsum_bins, flag, aggregate, inclusive_prefix, 
                              n_passes, i, bit_offset, n, grid_n, TILE_N, TILE_R, k_bits, descending)
